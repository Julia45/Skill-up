...
function wordsList(str, subStr) {
  var nakedString = str.replace(/[\s.,?!:'";]/g, ' ').toLowerCase()
  var eachWord = nakedString.split(' ')
  var newObject = {}
  for (var i = 0; i < eachWord.length; i++) {
    if (eachWord[i].includes(subStr)) {
      newObject[eachWord[i]] = eachWord[i];
    } else {
      continue
    }
  }
...
function getWeekDay(date) {
  ...
  var currentWeekDay = ourTestedDate.getDay()
...
function getDateAgo(presentDate, daysBack) {
  var miliSecondsInDay = 86400000;
  var convertedDate = new Date(presentDate)
